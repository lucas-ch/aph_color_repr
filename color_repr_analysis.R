...
plot_error_distribution_by_file <- function(df, sources) {
  ggplot(df
    ...
    labs(
      title = "Distribution des erreurs par fichier",
      x = "Erreur absolue (en degrés)",
      y = "Densité",
      color = "Fichier",
      fill = "Fichier"
    ) +
...
data_files = fichiers <- c(
  ...
  'aphantasia_lc_recall_with_text_1.csv',
  'aphantasia_lc_recall_with_text_2.csv',
  'aphantasia_lc_recall_with_text_3.csv',
...
plot_error_by_color(df, 'aphantasia_lc_recall_3')
plot_error_by_color(df, 'aphantasia_lc_recall_with_text_4')
plot_error_by_color(df, 'aphantasia_lc_match_without_color')

plot_color_description('data/aphantasia_lc_describe_2.csv')
